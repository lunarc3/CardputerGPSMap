...
# -*- coding: utf-8 -*-
"""
osm2tiles.py — OSM PBF → 瓦片图转换（OSM Carto 标准配色）

功能:
  1. power=* / man_made=* 全级别不渲染
  2. 道路/水系/铁路宽度全部按 zoom 缩放
  3. z6-7 精简: 仅 motorway+trunk+river+水域面+森林面
  4. z8 起逐步放开
  5. z11/z12 图标: 医院红十字、学校/大学房屋、火车站火车、机场飞机
  6. 每个瓦片内名称去重
  7. 块级 bbox 二进制扫描 + numpy 节点存储
  8. z6-7 仅 motorway+trunk; z8-11 仅 motorway+trunk+primary
  9. 去掉行政边界
  10. z6-10 水系仅 river+canal; stream 推迟到 z11
  11. 文字按 zoom+place 分级字号 + 1px 白色描边
  12. z6: 省名14px(admin_level=4) + 地级市圆点(admin_level=5)
  13. z7: 地级市名14px(admin_centre level=5, fallback level=4)
  14. z8-12: admin_centres level=5(地级市) + level=6(县级)
  15. z13+: place_nodes + admin_centres level=6 补充
  16. 大文件分阶段渲染: z6-8 / z9-12 标签过滤节点, z13-14 地理切分
"""
...
'''
    print("protobuf 模块未找到，正在自动生成...", flush=True)
    ok = False
    try:
    ...
    if not ok:
        ...
        try:
            ...
            pass
    if not ok:
        print("请执行: pip install protobuf grpcio-tools")
        sys.exit(1)
    import fileformat_pb2 as _ff, osmformat_pb2 as _of
    print("protobuf 模块已自动生成!", flush=True)
    return _ff, _of
        ...
        with open(filepath, 'rb') as f:
            ...
            while True:
                ...
                if now - tp >= progress_interval:
                    print(f"\r解析: {blk} 块, 跳过 {skipped}, "
                          f"节点 {nc:,}, 路径 {len(ways):,}, "
                          f"{now - t0:.0f}s", end="", flush=True)
                    tp = now
        if nodes is not None: nodes.finalize()
        print(f"\n解析完成: 节点 {nc:,}, 路径 {len(ways):,}, "
              f"关系 {len(relations):,}, place节点 {len(self.place_nodes):,}, "
              f"跳过 {skipped} 块, 耗时 {time.time() - t0:.1f}s", flush=True)
        return nodes, ways, relations
...
# ════════════════════════════════════════════════════════════════
# §7  render_region（z8-12 统一用 admin_centres）
# ════════════════════════════════════════════════════════════════
...
def render_region(nodes, ways, relations, zoom=14,
    ...
    print(f"路径: {len(ways):,}", flush=True)
    print("收集候选节点...", flush=True)
    t0 = time.time()
    wn = set()
    ...
    print(f"  过滤后: {len(fnids):,}  ({time.time() - t0:.1f}s)", flush=True)
    if not fnids: print("无节点", flush=True); return

    f_ids, f_lats, f_lons = nodes.get_coords_batch(fnids)
    if len(f_ids) == 0: print("无节点", flush=True); return
    ...
    print(f"画布: {cw}x{ch}, 瓦片: {ntx}x{nty}={ntx * nty}, "
          f"有数据: {len(occ)}, 全球起点: ({global_tx0},{global_ty0})",
          flush=True)

    print("构建节点→路径索引...", flush=True)
    ...
    print(f"  索引: {len(uniq):,} 节点, {len(pw):,} 关联, "
        ...
        for tile_x in range(ntx):
            ...
            if now - tp >= progress_interval:
                ...
                print(f"\r  {rendered}/{len(occ)}  "
                      f"{spd:.1f} 瓦片/秒  剩余 {rem:.0f}s ",
                      end="", flush=True)
                tp = now

    print(f"\n  渲染完成: {rendered} 瓦片, 跳过 {skipped}, "
          f"耗时 {time.time() - t0:.1f}s", flush=True)
    return output_dir
...
def _get_pbf_header_bbox(filepath):
    """从 PBF 文件头获取地理边界框 (south, west, north, east)"""
    try:
        with open(filepath, 'rb') as f:
            data, btype = OSMParser._read_blob(f)
            if data is None:
...
def _split_bbox(bbox, buf_deg=0.3, target_chunk_deg=15):
    """自适应切分 bbox:
    - 小范围(<10度)不切分
    - 中/大范围按 target_chunk_deg 动态计算 nx/ny
    返回 [(south, west, north, east), ...] 列表，每块带 buf 重叠
    """
    s, w, n, e = bbox
    width = e - w
    height = n - s
...
def _collect_needed_ids(filepath, max_zoom, bbox=None):
    """第一遍扫描: 收集指定 max_zoom 范围内需要的节点 ID 集合"""
    def way_filter(tags):
    ...
    for _, tags, nids in ways:
        ...
        way_ids.update(nids)
    if skipped_small:
        print(f"  跳过小面要素: {skipped_small:,}", flush=True)
    ...
    print(f"  way节点: {len(way_ids):,}, 行政中心: {len(ac_ids):,}, "
          f"合计: {len(needed):,}", flush=True)
    del way_ids, ac_ids
    needed_arr = np.fromiter(needed, dtype=np.int64)
    needed_arr.sort()
    print(f"  numpy: {needed_arr.nbytes / 1024 / 1024:.0f} MB "
          f"(set约 {len(needed) * 40 // 1024 // 1024} MB)", flush=True)
...
def _process_stage_two_pass(filepath, zooms, max_zoom, output_dir, args,
    ...
    """两遍模式阶段: 扫描 → 加载(过滤) → 渲染 → 释放"""
    print(f"\n{'=' * 60}", flush=True)
    print(f"  阶段 [{stage_label}]  (两遍模式)", flush=True)
    ...
    print(f"  数据: 节点 {len(nodes):,}, 路径 {len(ways):,}, "
          f"行政中心 {len(admin_centres)} ({ac_summary}), "
          f"省名 {len(province_names)}", flush=True)

    # ── 渲染 ──
    for z in zooms:
        print(f"\n--- 渲染 zoom {z} ---", flush=True)
    ...
    gc.collect()
    print(f"  ✓ 阶段完成, 已释放 {n_count:,} 节点内存", flush=True)


def _process_stage_chunk(filepath, zooms, max_zoom, output_dir, args,
                          bbox=None, stage_label=""):
    """单遍模式阶段（用于 bbox 切分后的每个小块）: 全量加载 → 渲染 → 释放"""
    def way_filter(tags):
        return _way_zoom_ok(tags, max_zoom)

    print(f"\n{'=' * 60}", flush=True)
    print(f"  阶段 [{stage_label}]  (单遍模式)", flush=True)
    ...
    print(f"  节点 {len(nodes):,}, 路径 {len(ways):,}, "
          f"行政中心 {len(admin_centres)} ({ac_summary}), "
          f"省名 {len(province_names)}", flush=True)
    ...
    del nodes, ways, relations, admin_centres, province_names, place_nodes
    gc.collect()
    print(f"  ✓ 块完成, 已释放 {n_count:,} 节点内存", flush=True)
...
def _split_bbox(bbox, nx=4, ny=3, buf=0.3):
    """将 bbox 切成 nx*ny 个子块，每块向外扩展 buf 度重叠"""
    s, w, n, e = bbox
    lat_step = (n - s) / ny
    lon_step = (e - w) / nx
    chunks = []
...
def main():
    ap = argparse.ArgumentParser(description="OSM PBF -> 瓦片图")
    ap.add_argument("input", help="输入 .osm.pbf 文件")
    ap.add_argument("-z", "--zoom", default=None,
                    help="缩放级别 (如 14 或 10-12)，不指定则交互输入")
    ap.add_argument("-b", "--bbox", default=None,
                    help="边界框: south,west,north,east")
    ap.add_argument("-o", "--output", default="gpsmap", help="输出目录")
    ...
    if args.zoom is not None:
        ...
        z_input = input("请输入起始zoom级别 [默认10]: ").strip()
        start_z = int(z_input) if z_input else 10
        z_input = input("请输入结束zoom级别 [默认12]: ").strip()
    ...
    print(f"将渲染 zoom: {', '.join(str(z) for z in zooms)}", flush=True)
    ...
    if args.single_pass or fsz < 200 * 1024 * 1024:
        print(f"\n小文件/单遍模式 ({fsz / 1024 / 1024:.0f} MB)", flush=True)
        ...
        if bbox:
            ...
            if _osmium_extract(fp, bbox, temp_file):
                ...
                print(f"osmium 提取成功: {tsz / 1024 / 1024:.0f} MB "
                      f"(原 {fsz / 1024 / 1024:.0f} MB)", flush=True)
                actual_input = temp_file
                bbox = None
            else:
                print("osmium 不可用，使用 Python bbox 解析...", flush=True)
        ...
        print(f"\n数据: 节点 {len(nodes):,}, 路径 {len(ways):,}, "
              f"关系 {len(relations):,}, place节点 {len(place_nodes):,}, "
              f"行政中心 {len(admin_centres)} ({ac_summary}), "
              f"省名 {len(province_names)}", flush=True)

        for z in zooms:
            print(f"\n{'=' * 50}", flush=True)
            print(f"  渲染 zoom {z}", flush=True)
    ...
    # 大文件分阶段渲染
    # ═══════════════════════════════════════════════════
    print(f"\n大文件分阶段模式 ({fsz / 1024 / 1024:.0f} MB)", flush=True)
    ...
    if bbox:
        ...
        if file_bbox:
            print(f"PBF 文件头 bbox: S={file_bbox[0]:.2f} "
                  f"W={file_bbox[1]:.2f} N={file_bbox[2]:.2f} "
                  f"E={file_bbox[3]:.2f}", flush=True)
        else:
            print("无法获取文件 bbox，z13-14 将使用全量加载", flush=True)
    ...
    if high_zooms:
        ...
        if file_bbox:
            ...
            if len(chunks) == 1:
                ...
                print(f"\nz{min(high_zooms)}-{max(high_zooms)} "
                      f"地理切分: {len(chunks)} 块", flush=True)
